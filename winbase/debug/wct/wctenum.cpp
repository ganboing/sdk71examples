...
--*/
{
    ...
    if (!AdjustTokenPrivileges(TokenHandle,
                               ...
                               NULL))
    {
        printf("Couldn't revoke debug privilege");
        goto Cleanup;
    }
...
--*/
{
    ...
    printf("\t (no params) -- gets wait chain for all threads\n");
    printf("\t ProcId      -- gets wait chain for the specified process\n\n");
}
...
--*/
{
    ...
    // Initialize the WCT interface to COM.  We will continue if this
    // fails--just won't get COM information.
    if (!InitCOMAccess())
    {
